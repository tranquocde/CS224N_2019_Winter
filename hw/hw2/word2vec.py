...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    scores = outsideVectors.dot(centerWordVec) #(vocab_size,1)
    probs = softmax(scores) #(vocav_size,1) y_hat
    ...
    gradCenterVec = outsideVectors.T.dot(dscores)# (embedding_dim,1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    loss = 0.0
    gradCenterVecs = np.zeros(centerWordVectors.shape) #dJ/dV
    gradOutsideVectors = np.zeros(outsideVectors.shape) #dJ/dU
